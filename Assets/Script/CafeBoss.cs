...
public class CafeBoss : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        else if (isit == 1)//플레이어를 노릴 수 있는 위치라면
        {
            ...
            {
                transform.position = Vector3.MoveTowards(transform.position, target2.position, goSpeed * Time.deltaTime);//플레이어를 향해 돌진
            }
        }
        else if (isit == 2)
        {
